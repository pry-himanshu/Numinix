...
import { DiagnosticResult, StudyPlan, ChapterProgress } from '../../types';
import { generatePersonalizedStudyPlan, generateChapterExplanation, generateSmartNotification } from '../../services/studyPlanService';

interface EnhancedMathMapProps {
  diagnosticResult?: DiagnosticResult;
}

export function EnhancedMathMap({ diagnosticResult }: EnhancedMathMapProps) {
  ...
  const [smartNotification, setSmartNotification] = useState<string>('');
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    if (userProfile && diagnosticResult) {
  ...
  const generateSmartNotifications = async () => {
    ...
    const recentActivity = [
      `Completed diagnostic test with ${diagnosticResult.score}/${diagnosticResult.total_questions} score`,
      `Strong in: ${diagnosticResult.strengths.join(', ')}`,
      `Areas to improve: ${diagnosticResult.weaknesses.join(', ')}`
    ];
    
    const notification = await generateSmartNotification(userProfile, recentActivity);
  ...
  const getChapterStatus = (chapter: any, index: number) => {
    ...
    const prevProgress = chapterProgress[previousChapter?.id];
    
    if (prevProgress && prevProgress.completion_percentage >= 70) {
      return 'available';
    }
  ...
  const handleChapterClick = async (chapter: any) => {
    ...
    try {
      ...
      if (!studyPlans[chapter.id] && diagnosticResult) {
      ...
      if (!explanations[chapter.id]) {
        const studentLevel = diagnosticResult && diagnosticResult.score / diagnosticResult.total_questions >= 0.7 
          ? 'intermediate' 
          : 'beginner';
        ...
        {smartNotification && (
          <div className="bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 rounded-2xl p-6 mb-8 shadow-2xl border border-white/20">
            <div className="flex items-center space-x-3">
              ...
              <p className="text-white font-semibold text-lg">{smartNotification}</p>
            </div>
          </div>
        )}
            ...
            <div className="bg-black/30 backdrop-blur-xl rounded-2xl p-6 shadow-xl border border-white/20">
              ...
              <div className="space-y-2">
                {diagnosticResult.strengths.length > 0 ? (
                  ...
                  <div className="text-gray-400 text-sm">Keep practicing to build strengths!</div>
                )}
              </div>
            </div>
              ...
              <div className="space-y-2">
                {diagnosticResult.weaknesses.length > 0 ? (
                  diagnosticResult.weaknesses.map((weakness, index) => (
                    <div key={index} className="bg-yellow-500/20 text-yellow-300 px-3 py-2 rounded-lg text-sm">
                      ⚡ {weakness}
                    </div>
                  ))
                ) : (
                  <div className="text-gray-400 text-sm">Great! No major weak areas identified.</div>
                )}
              </div>
            </div>
            ...
            <div className="bg-black/30 backdrop-blur-xl rounded-2xl p-6 shadow-xl border border-white/20">
              ...
              <div className="space-y-2">
                <div className="bg-blue-500/20 text-blue-300 px-3 py-2 rounded-lg text-sm">
                  📌 Start with {sortedChapters[0]?.chapter}
                </div>
            ...
            {sortedChapters.map((chapter, index) => {
              ...
              return (
                <div
                  ...
                  onClick={() => isClickable && handleChapterClick(chapter)}
                >
                    ...
                    {chapterProgress[chapter.id] && (
                      <div className="mt-4">
                        <div className="flex justify-between text-sm text-gray-400 mb-2">
                          <span>Progress</span>
                          <span>{Math.round(chapterProgress[chapter.id].completion_percentage)}%</span>
                        </div>
                        <div className="w-full bg-gray-700 rounded-full h-2">
                          <div
                            className="bg-gradient-to-r from-blue-400 to-purple-500 h-2 rounded-full transition-all duration-300"
                            style={{ width: `${chapterProgress[chapter.id].completion_percentage}%` }}
                          />
                        </div>
                      </div>
                    )}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                          ...
                          <ul className="space-y-2">
                            {studyPlans[selectedChapter].prerequisites.map((prereq, index) => (
                        ...
                        <div>
                          ...
                          <ul className="space-y-2">
                            {studyPlans[selectedChapter].recommended_practice.map((practice, index) => (
                              <li key={index} className="text-blue-100 flex items-center space-x-2">
